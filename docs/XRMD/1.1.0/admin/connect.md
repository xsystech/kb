...
В данном разделе описывается порядок регистрации брокеров OpenUDS в XRM Director и базовые операции управления ими.

***

### 1. Переход к разделу настройки брокеров

Для подключения брокеров администратор открывает раздел настройки брокеров в интерфейсе XRM Director.
...
* управление брокерами доступно из интерфейса XRM Director.

Если система позволяет выделить брокер в таблице и выбрать действие `Управление`, это означает, что XRM Director может использовать зарегистрированную запись для перехода к управлению выбранным брокером.

***

### 5. Использование команды `Управление`

Ключевая административная возможность XRM Director — открытие веб-интерфейса брокера непосредственно из системы.
...
После открытия веб-интерфейса брокера в верхней части страницы отображается служебная панель XRM Director. Она показывает, что администратор работает с брокером через XRM Director, а не выполняет прямой вход на площадку вручную.

На этой панели отображаются:

* **Broker ID** – внутренний идентификатор брокера в XRM Director;
...
* подтверждение доступности обеих площадок через XRM Director;
